...
class MiniCourt:
    ...
    def get_minicourt_coordinates(self,object_position,closest_keypoint,closest_keypoint_index,player_height_in_pixels,player_height_in_metres):
        distance_from_keypoint_x_pixels,distance_from_keypoint_y_pixels = measure_xy_distance(object_position,closest_keypoint)

        distance_from_keypoint_x_metres = convert_pixel_distance_to_metres(distance_from_keypoint_x_pixels,constants.DOUBLE_LINE_WIDTH,self.court_drawing_width)

        distance_from_keypoint_y_metres = convert_pixel_distance_to_metres(distance_from_keypoint_y_pixels,constants.DOUBLE_LINE_WIDTH,self.court_drawing_width)

        minicourt_x_distance_pixels = self.convert_metres_to_pixels(distance_from_keypoint_x_metres)
        minicourt_y_distance_pixels = self.convert_metres_to_pixels(distance_from_keypoint_y_metres)

        closest_minicourt_keypoint = (self.drawing_key_points[closest_keypoint_index*2],self.drawing_key_points[closest_keypoint_index*2+1])
        minicourt_player_position = (closest_minicourt_keypoint[0]+minicourt_x_distance_pixels,closest_minicourt_keypoint[1]+minicourt_y_distance_pixels)

        return minicourt_player_position
    
    def convert_bboxes_to_minicourt_coordinates(self,player_boxes,ball_boxes,original_court_keypoints):
        player_heights = {
        ...
        for frame_num,player_bbox in enumerate(player_boxes):
            ...
            closest_player_id_to_ball = min(player_bbox.keys(),key = lambda x: measure(ball_position,get_center(player_bbox[x])))
            ...
            for player_id,bbox in player_bbox.items():
                ...
                minicourt_player_position = self.get_minicourt_coordinates(foot_position,closest_keypoint,closest_keypoint_index,max_player_height_in_pixels,player_heights[player_id])

                output_player_boxes[player_id] = minicourt_player_position

                if closest_player_id_to_ball == player_id:
                     closest_keypoint_index = get_closest_keypoint_index(ball_position,original_court_keypoints,[0,2,12,13])
                     closest_keypoint = (original_court_keypoints[closest_keypoint_index*2],original_court_keypoints[closest_keypoint_index*2+1])

                     minicourt_player_position = self.get_minicourt_coordinates(ball_position,closest_keypoint,closest_keypoint_index,max_player_height_in_pixels,player_heights[player_id])
